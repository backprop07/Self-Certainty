...
@torch.no_grad()
def confidence_logits(logits: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    """
    Calculate the self-certainty score for the provided logits using the Kullback-Leibler divergence.

    Args:
        logits (torch.Tensor): Tensor of shape (batch_size, seq_length, vocab_size) or (seq_length, vocab_size).
        attention_mask (torch.Tensor): Tensor of shape (batch_size, seq_length) or (seq_length) indicating valid tokens.

    Returns:
        torch.Tensor: A 1D tensor of confidence scores for valid tokens.
    """
...
@torch.no_grad()
def confidence_with_file(filepath: str, output_file: str = None, batch_size: int = 8) -> None:
    """
    Process an input JSON file to compute confidence scores for each output generated by a language model.
    The function loads a pretrained model and tokenizer, computes confidence scores for each output sequence,
    and writes the updated data with confidence scores to an output JSON file.

    Args:
        filepath (str): Path to the input JSON file.
        output_file (str, optional): Path to the output JSON file. If not provided, a default name is generated.
        batch_size (int): Number of sequences to process simultaneously. Defaults to 8.

    Raises:
        ValueError: If the input JSON does not contain required keys ('generator', 'output', or 'model_input').
    """
    ...
    if "output" not in data[0]:
        raise ValueError("The input file does not have output attribute")
    if "model_input" not in data[0]:
        raise ValueError("The input file does not have model_input attribute")
    ...
    for index in tqdm.tqdm(range(processed_items, total_items), desc="Processing inputs"):
        ...
        torch.cuda.empty_cache()
    print(f"All items processed. Output written to {output_file}")
